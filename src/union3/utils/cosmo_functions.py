...
def get_colors(key):
    if key == "blue":  # SNe
        return (
            (0.0, 83 / 255.0, 152 / 255.0),
            (30 / 255.0, 104 / 255.0, 168 / 255.0),
            (92 / 255.0, 140 / 255.0, 190 / 255.0),
        )

    if key == "orange":  # CMB
        return (
            (243 / 255.0, 116 / 255.0, 17 / 255.0),
            (248 / 255.0, 144 / 255.0, 62 / 255.0),
            (250 / 255.0, 180 / 255.0, 110 / 255.0),
        )

    if key == "blueorange":
        return [(111 / 255.0, 0.0, 255.0 / 255.0)] * 3
    if key == "bluegreen":
        return [(0.0, 65 / 255.0, 201 / 255.0)] * 3
    if key == "greenorange":
        return [(255 / 255.0, 177 / 255.0, 0.0)] * 3

    if key == "green":  # BAO
        return (
            (0.0, 160 / 255.0, 52 / 255.0),
            (50 / 255.0, 176 / 255.0, 86 / 255.0),
            (117 / 255.0, 198 / 255.0, 126 / 255.0),
        )

    if key == "gray":  # Combined
        return (
            (71 / 255.0, 71 / 255.0, 71 / 255.0),
            (119 / 255.0, 119 / 255.0, 119 / 255.0),
            (178 / 255.0, 178 / 255.0, 178 / 255.0),
        )

    if key == "teal":  # Combined, w0-wa
        return (
            (0 / 255.0, 168 / 255.0, 168 / 255.0),
            (127 / 255.0, 204 / 255.0, 189 / 255.0),
            (190 / 255.0, 226 / 255.0, 210 / 255.0),
        )
    assert 0, key
...
def get_DETF(the_grid):
    ...
    for cut_val in np.linspace(6.15, 6.21, 100):
        ...
        plt.plot(cut_val, 1.0 / included_area, ".", color="b")
    plt.axvline(6.18007)
...
def get_sound_speed(z, O_bhh):
    return 1.0 / np.sqrt(3.0 * (1.0 + 0.75 * O_bhh / CosConst.O_gammahh / (1.0 + z)))


def getzstar(O_m, hh, O_bhh):
    g1 = (0.0783 * (O_bhh) ** (-0.238)) / (1.0 + 39.5 * O_bhh**0.763)
    g2 = 0.560 / (1.0 + 21.1 * O_bhh**1.81)

    # ztar is CMB redshift
    zstar = 1048.0 * (1.0 + 0.00124 * O_bhh ** (-0.738)) * (1 + g1 * (O_m * hh) ** g2)
...
def getzdrag(O_m, hh, O_bhh):
    ...
    b1 = 0.313 * (O_m * hh) ** (-0.419) * (1.0 + 0.607 * (O_m * hh) ** (0.674))
    b2 = 0.238 * (O_m * hh) ** 0.223

    zdrag = 1291 * (O_m * hh) ** (0.251) * (1.0 + b1 * (O_bhh) ** b2) / (1.0 + 0.659 * (O_m * hh) ** 0.828)
...
def highz_r(cosmo, zmin, zmax, sound=0):  # use log(1 + z) instead of z to to the integral-- faster for large z
    ...
    r = 0.0
    for i in range(n - 1):
        r += (zint_list[i + 1] - zint_list[i]) * (Hinvlist[i] + 4.0 * Hinvlist_inter[i] + Hinvlist[i + 1]) / 6.0
...
def Planck18_CMB_chi2(cosmo, merged_mat):
    ...
    z_star = getzstar(O_m=cosmo["O_m"], hh=cosmo["h"] ** 2.0, O_bhh=cosmo["O_bhh"])
    r_star = get_sound_horizon(cosmo=cosmo, CMB_not_BAO=1)
    theta = 100 * r_star / highz_r(cosmo=cosmo, zmin=0.0, zmax=z_star)
...
def get_DV(z, cosmo):
    ...
    DV = (r**2.0 * z * get_Hinv(z, cosmo)) ** (1.0 / 3.0)
    return DV
...
def get_BAO_chi2(BAO_data, cosmo, verbose=False):
    ...
    for i in range(len(BAO_data["constraint"])):
        if BAO_data["constraint"][i] == "DV":
            ...
            this_val = get_H(BAO_data["redshift"][i], cosmo) * rs * CosConst.c100_Mpc * 100.0  # h cancels out
            this_val /= BAO_data["fiducial"][i]
        else:
            assert 0
...
def n_integrate(x_list, intfn, otherargs, pad_list=np.arange(41, dtype=np.float64) / 20.0):
    nadded = len(pad_list)
    ...
    for i in range(1, len(intfn_sort)):
        the_integral[i] = (
            the_integral[i - 1]
            + (intfn_sort[i - 1] + 4.0 * intfn_sort_inter[i - 1] + intfn_sort[i])
            * (x_sort_list[i] - x_sort_list[i - 1])
            / 6.0
        )

    tmplist = the_integral * 1.0
...
def get_Hinv(z_list, cosmo):
    omega_r = CosConst.O_radhh / cosmo["h"] ** 2.0

    if cosmo["model"] == "LCDM":
        return 1.0 / np.sqrt(
            omega_r * (1.0 + z_list) ** 4.0
            + cosmo["O_m"] * (1.0 + z_list) ** 3.0
            + (1.0 - cosmo["O_m"] - cosmo["O_k"] - omega_r)
            + cosmo["O_k"] * (1.0 + z_list) ** 2.0
        )

    if cosmo["model"] == "flatLCDM":
        return 1.0 / np.sqrt(
            omega_r * (1.0 + z_list) ** 4.0 + cosmo["O_m"] * (1.0 + z_list) ** 3.0 + (1.0 - cosmo["O_m"] - omega_r)
        )
    ...
    if cosmo["model"] == "flatwCDM":
        return 1.0 / np.sqrt(
            omega_r * (1.0 + z_list) ** 4.0
            + cosmo["O_m"] * (1.0 + z_list) ** 3.0
            + (1.0 - cosmo["O_m"] - omega_r) * (1 + z_list) ** (3.0 * (1.0 + cosmo["w"]))
        )
        ...
        return 1.0 / np.sqrt(
            ...
            + cosmo["O_k"] * (1.0 + z_list) ** 2.0
            + (1.0 - cosmo["O_m"] - cosmo["O_k"] - omega_r)
            * np.exp(-3.0 * cosmo["w_a"] * z_list / (1.0 + z_list))
            * (1.0 + z_list) ** (3.0 * (1.0 + cosmo["w_0"] + cosmo["w_a"]))
        )

    elif cosmo["model"] == "binnedrho":
        DE_density = (z_list <= cosmo["zbins"][0]) * (1.0 - cosmo["O_m"] - cosmo["O_k"] - omega_r)
        ...
        return 1.0 / np.sqrt(
            ...
            + O_k * (1.0 + z_list) ** 2.0
            + O_x
            * exp(-3.0 * w_a * z_list / (1.0 + z_list))
            * (1.0 + z_list) ** (3.0 * (1.0 + w_p + w_a / (1.0 + z_p)))
        )
    else:
...
def get_dt(z_list, cosmo):
    return get_Hinv(z_list, cosmo) / (1.0 + z_list)
...
def get_mu(z_list, cosmo, z_helio_list=[-2]):
    ...
    return (
        5.0 * np.log10((1.0 + z_helio_list) * sinn_r(r_list, cosmo["O_k"])) + CosConst.cH100 - 5 * np.log10(cosmo["h"])
    )
...
def no_big_bang(Om):  # For Omega_m-Omega_Lambda
    return (
        1
        - Om
        + (3 * Om ** (4.0 / 3.0)) / (2.0 * (1 + np.sqrt(1 - 2 * Om) - Om) ** (1.0 / 3.0))
        + (3 * ((1 + np.sqrt(1 - 2 * Om) - Om) * Om**2) ** (1.0 / 3.0)) / 2.0
    )


assert np.isclose(no_big_bang(0.1), 27.0 / 20)
assert np.isclose(no_big_bang(0.5), 2.0)


class CosConst:
    c = 299792458.0  # c / (100 km/s/Mpc) / (10 pc) = 299792458
    c100_Mpc = 2997.92458
    cH100 = 5.0 * np.log10(c)
    cH70 = cH100 - 5.0 * np.log10(0.7)
    Hinv100 = 9.77813  # Gyrs

    O_gammahh = 0.000024729  # 2.469e-5 # Komatsu 2009
    O_radhh = (1.0 + 0.2271 * 3.04) * O_gammahh  # Komatsu 2009
