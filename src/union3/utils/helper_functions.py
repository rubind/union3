...
from union3.file_read import writecol
...
def get_params(paramfl):
    ...
    params["IG_fn"] = RectBivariateSpline(IG_dat[1:, 0], IG_dat[0, 1:], IG_dat[1:, 1:], kx=1, ky=1)
...
def get_dparam_dzps(res_der_fl, redshift, calibration_paths):
    wavebins = [
        ...
        (8000.0, 100000.0),
        (10000.0, 100000.0),
    ]  # 4000 and 8000 for the breaks, roughly, 10000 for STIS -> WFC3/NICMOS

    Landolt_Smith_bins = dict(
        L={
            "LANDOLT_U": (3000, 4000),
            "LANDOLT_B": (4000, 5000),
            "LANDOLT_V": (5000, 6000),
        ...
        P={"PS1_g": (4000, 5500), "PS1_r": (5500, 7000), "PS1_i": (7000, 8000), "PS1_z": (8000, 10000)},
    )
            ...
            for Landolt_or_Smith in "LSP":
                if calibration_paths[the_key] == Landolt_or_Smith:
                    for LSkey in Landolt_Smith_bins[Landolt_or_Smith]:
                        if (Landolt_Smith_bins[Landolt_or_Smith][LSkey][0] < obslamb) and (
                            Landolt_Smith_bins[Landolt_or_Smith][LSkey][1] > obslamb
                        ):
                            ...
                            if key_to_write in dparam_dzps:
                                dparam_dzps[key_to_write] += array(
...
def get_MWEBV_uncs(lightfl, res_der_fl, params):
    ...
    d_dMWEBV = array(
        [
            read_param(res_der_fl, "MWEBV", ind=5),
    ...
    extra_cmat = outer(
        MWEBV * sig_stat * d_dMWEBV, MWEBV * sig_stat * d_dMWEBV
    )  # Has the same mB, x1, c order as LC covariance matrices
...
def get_electron_scattering(redshift, params):
    ...
    tau_electron = params["electron_coeff"][0] * (sqrt(0.3 * (1 + redshift) ** 3.0 + 0.7) - 1.0)
    dtau_electron = params["electron_coeff"][1] * (sqrt(0.3 * (1 + redshift) ** 3.0 + 0.7) - 1.0)

    add_mag_electron = -2.5 / log(10.0) * tau_electron
    dmag_electron = 2.5 / log(10.0) * dtau_electron
...
def samples_txt_to_pickle(flname, samples_to_burn, skip=6):
            ...
            for j in range(len(parsed)):
                ...
                if parsed[j].count(".") == 0:
                    ...
                    if new_key:
                        ...
                        headings_size[pparsed[0]] = (
                            max(headings_size[pparsed[0]][0], int(pparsed[1])),
    ...
    for key in headings_size:
        if headings_size[key] == 0:
            samples[key] = zeros(samples_count, dtype=float64) - 1.0e20
        else:
            samples[key] = zeros((samples_count,) + headings_size[key], dtype=float64) - 1.0e20
...
def remap_x1(x1, params):
    ...
    new_x1 = x1 + float(params["remap_x1"][0]) * x1**2.0 + float(params["remap_x1"][1]) * x1**3.0
    x1_slope = 1.0 + 2.0 * float(params["remap_x1"][0]) * x1 + 3 * float(params["remap_x1"][1]) * x1**2.0
...
def write_latent_variables(the_data, stan_data, fit_params):
    ...
    data_to_write = [
        [item.split("/")[-1] for item in the_data["snpaths"]],
    ...
    writecol("latent_table.txt", data_to_write, headings=headings)
...
def gelman_rubin_R(samples):
    ...
    s_j2 = 1.0 / (n - 1.0) * sum((samples - psi_dotj) ** 2.0)

    B = n / (m - 1.0) * sum((psi_dotj - psi_dotdot) ** 2.0)
    W = (1.0 / m) * sum(s_j2)
...
def filter_fit_params(fit_params, param_name, chains, iter_per_chain):
    ...
    good_chains = stdevs > max(stdevs) / 4.0
...
def quick_print(vals, thename):
    txt = "  ".join(
        [
            ...
            "Mean 15.9 50 84.1",
            "%.4f" % mean(vals),
            "%.4f" % scoreatpercentile(vals, 15.8655),
            "%.4f" % scoreatpercentile(vals, 50),
            "%.4f" % scoreatpercentile(vals, 84.1345),
        ]
    )
    return txt
    ...
    for samp_cat_target in range(len(redshift_bins)):
        for i in range(stan_data["n_samples"]):
            ...
            if samp_cat == samp_cat_target:
                ...
                samp_bins["labels"].append(lines[ind][1].replace("_", "\n"))
