...
class StanModel(Model):
    ...
    def initialise(self, data: Data) -> None:
        ...
        self.data = {
            ...
            "photo_z0": snia["photo_z0"].drop_nulls().to_numpy().astype(np.float64),
            "photo_sigz": snia["photo_sigz"].drop_nulls().to_numpy().astype(np.float64),
            "photo_spikez": snia["photo_spikez"].drop_nulls().to_numpy().astype(np.float64),
            "spike_redshift_prob": snia["photo_pspike"].drop_nulls().to_numpy().astype(np.float64),
            "photoz_inds": snia["photoz_index"].to_numpy().astype(np.int32),
            "est_mobs_cuts": data.samples["est_mobs_cuts"].to_numpy().astype(np.float64),
            "est_mobs_sigmas": data.samples["est_mobs_sigmas"].to_numpy().astype(np.float64),
            "mobs_cut0": snia["mobs_cut0"].to_numpy().astype(np.float64),
            "mobs_cut1": snia["mobs_cut1"].to_numpy().astype(np.float64),
            "BAOCMB_Om_w0_wa_mean": data.bao_cmb_omw0wa["mean"].astype(np.float64),
            "BAOCMB_Om_w0_wa_covmatrix": data.bao_cmb_omw0wa["cov"].astype(np.float64),
            "n_zbins": len(data.redshift_bins["zbins"]),
            "zbins": data.redshift_bins["zbins"].astype(np.float64),
            "dmu_dbin": data.redshift_bins["dmu_dbin"].astype(np.float64),
            "dmudz_dbin": data.redshift_bins["dmudz_dbin"].astype(np.float64),
            "mu_const": snia["mu_const"].to_numpy().astype(np.float64),
            "dmu_const_dz": snia["dmu_const_dz"].to_numpy().astype(np.float64),
        }
    ...
    def fit(self) -> pl.DataFrame:
        ...
        logger.info(f"Starting Stan MCMC sampling for {self.config.iterations} iterations...")
