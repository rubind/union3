...
for i in tqdm.tqdm(range(len(pfls))):
    ...
    plt.plot(plot_pos + 0.1*i, median(MB_high_mass, axis = 0), 'os*.v^'[i], label = labels[pfls[i].split("/")[0]], color = colors[i])
    ...
    for j in range(len(p16)):
        ...
        plt.plot([plot_pos + 0.1*i]*2, [p16[j], p84[j]], color = colors[i])

        plt.text(plot_pos, -18.98 - blinding_const, ["CSP", "CfA1", "CfA2", "CfA3", "CfA4"][j])# 4S", "CfA3 KC", "CfA4"][j])
yticks = plt.yticks()[0]
